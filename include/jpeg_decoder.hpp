...
// MiniJPEGDecoder 1.0.1
//
// Single-C++-Header JPEG Decoder, originall by Scott Graham, based on
// Martin J. Fiedler's NanoJPEG library. Found on
// <https://scot.tg/2009/12/02/mini-jpeg-decoder/>
// Modifications:
//   Version 1.0.1:
//     * Gate register-usage by cpp version
//
// Original header comment:
//
// This is a cosmetic restructing and port to C++ class of 'NanoJPEG', found
// at <page hijacked -> link removed >. It's been made somewhat thread safe in
// that all context information is pulled into an object, rather than being
// global as the original was. Other than that, the original is superior in
...
#    define JPEG_DECODER_REGISTER register
#else
#    define JPEG_DECODER_REGISTER
#endif
...
#define JPEG_DECODER_THROW(e)                                                                      \
    ...
    inline void _DecodeSOF(void)
    {
        ...
        switch (ctx.ncomp)
        {
            case 1:
            case 3: break;
            default: JPEG_DECODER_THROW(Unsupported);
        }
        ...
        for (i = 0, c = ctx.comp; i < ctx.ncomp; ++i, ++c)
        {
            ...
            if (!(c->ssx = ctx.pos[1] >> 4))
                JPEG_DECODER_THROW(SyntaxError);
            if (c->ssx & (c->ssx - 1))
                JPEG_DECODER_THROW(Unsupported); // non-power of two
    ...
    inline void _DecodeDHT(void)
    {
        ...
        while (ctx.length >= 17)
        {
            ...
            for (codelen = 1; codelen <= 16; ++codelen)
            {
                ...
                for (i = 0; i < currcnt; ++i)
                {
                    JPEG_DECODER_REGISTER unsigned char code = ctx.pos[i];
                    for (j = spread; j; --j)
                    {
                    ...
                    for (sby = 0; sby < c->ssy; ++sby)
                        for (sbx = 0; sbx < c->ssx; ++sbx)
                        {
                            _DecodeBlock(
                                c,
                                &c->pixels
                                     [((mby * c->ssy + sby) * c->stride + mbx * c->ssx + sbx)
                                      << 3]);
                            if (ctx.error)
                                return;
                        }
    ...
    inline void _Convert()
    {
        ...
        for (i = 0, c = ctx.comp; i < ctx.ncomp; ++i, ++c)
        {
            ...
            if ((c->width < ctx.width) || (c->height < ctx.height))
                JPEG_DECODER_THROW(InternalError);
        }
        if (ctx.ncomp == 3)
        {
            // convert to RGB
            ...
            for (yy = ctx.height; yy; --yy)
            {
                ...
                {
                    JPEG_DECODER_REGISTER int y  = py[x] << 8;
                    JPEG_DECODER_REGISTER int cb = pcb[x] - 128;
                    JPEG_DECODER_REGISTER int cr = pcr[x] - 128;
...
inline Decoder::Decoder(
    const unsigned char* data, size_t size, void* (*allocFunc)(size_t), void (*freeFunc)(void*))
    : AllocMem(allocFunc)
    , FreeMem(freeFunc)
{
    // should be static data, but this keeps us as a header
    char temp[64] = { 0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32, 25, 18, 11, 4,  5,
